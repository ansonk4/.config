...
`)}`)}}};var y=require("node:child_process");async function T(e){if(process.platform!=="darwin")throw new Error("macOS only");let t=process.env.LC_ALL;delete process.env.LC_ALL;let{stdout:n,stderr:s}=(0,y.spawnSync)("osascript",["-e",e]);if(process.env.LC_ALL=t,s?.length)throw new Error(s.toString());return n.toString()}async function $(e){let t=`
  ...
  `;try{let n=await T(t);await(0,u.showToast)(u.Toast.Style.Success,"Done",n)}catch(n){await g(n)}}var _=async()=>await $("WezTerm");
